      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-10">
        <div>
          <div className="flex items-center gap-2.5 text-red-500 mb-2.5">
            ...
            <span className="text-[11px] font-bold uppercase tracking-[0.3em]">Incidents</span>
          </div>
          ...
          <p className="text-muted-foreground mt-4 max-w-lg font-medium leading-relaxed">Real-time incident response and forensic alert triaging.</p>
        </div>
        <div className="flex bg-card border border-border p-1 rounded-xl shadow-sm">
          {(['pending', 'acknowledged', 'all'] as const).map((t) => (
            <button
              key={t}
              onClick={() => setFilter(t)}
              className={`px-6 py-2 rounded-lg text-[11px] font-bold uppercase tracking-widest transition-all ${filter === t ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-muted-foreground hover:text-foreground'}`}
            >
              {t}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        {stats.map(s => (
          <div key={s.label} className="bg-card border border-border rounded-2xl p-6 flex items-center justify-between shadow-sm">
            <span className="text-[11px] font-bold uppercase text-muted-foreground tracking-widest">{s.label}</span>
      ...
      <div className="space-y-6">
        {loading ? (
          ...
          <div className="bg-card border border-border rounded-[40px] p-20 text-center relative overflow-hidden group shadow-sm">
            ...
            <h3 className="text-xl font-bold text-foreground uppercase tracking-widest mb-2">Perimeter Secure</h3>
            <p className="text-muted-foreground text-sm max-w-xs mx-auto italic font-medium">No unacknowledged security incidents require your immediate attention.</p>
          </div>
        ) : (
          alerts.map((alert) => (
            <div 
              ...
              <div className="flex flex-col lg:flex-row lg:items-start gap-8">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-4 mb-4">
                    <span className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-lg border shadow-sm ${getSeverityClass(alert.severity)}`}>
                      {alert.severity}
                    </span>
                    <span className="text-muted-foreground/20">|</span>
                    <span className="text-[11px] font-bold text-muted-foreground/60 uppercase tracking-tight truncate max-w-[200px]">{alert.threat?.sender}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-8 pt-6 border-t border-border">
                    <div className="flex items-center gap-2.5 text-[10px] font-bold uppercase text-muted-foreground/40 tracking-widest">
                      <Clock className="w-4 h-4" />
                      {new Date(alert.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </div>
                    <div className="flex items-center gap-2.5 text-[10px] font-bold uppercase text-muted-foreground/40 tracking-widest">
                      <Activity className="w-4 h-4" />
                      Risk Index: <span className="text-foreground font-mono">{alert.threat?.risk_score}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row lg:flex-col gap-3 min-w-[180px]">
                  {!alert.acknowledged ? (
                    <button
                      ...
                      className="bg-primary hover:bg-blue-600 text-white px-6 py-4 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all shadow-lg shadow-primary/10 flex items-center justify-center gap-3 group/btn"
                    >
                      Acknowledge
                      <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
                    </button>
                  ) : (
                    <div className="flex items-center justify-center gap-2.5 text-emerald-500/60 text-[11px] font-bold uppercase tracking-widest italic px-6 py-4 bg-emerald-500/5 rounded-xl border border-emerald-500/10">
